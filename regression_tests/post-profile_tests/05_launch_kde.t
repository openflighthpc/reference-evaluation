Check that you can launch kde
  $ flight desktop start kde
  Starting a '\x1b[36mkde\x1b[0m' desktop session: (esc)
  
     > \xe2\x9c\x85 \x1b[38;2;39;148;216mStarting session\x1b[0m (esc)
  
  A '\x1b[36mkde\x1b[0m' desktop session has been started. (esc)
  Identity\t* (esc) (glob)
  Type\tkde (esc)
  Host IP\t* (esc) (glob)
  Hostname\t* (esc) (glob)
  ...
  $ flight desktop list | grep "kde" | grep "Active"
  *\tkde\t*\t*\t1\t*\t41361\t*\tActive\t*\t*\t/home/flight/.cache/flight/desktop/sessions/*/session.png\t*|*\t\t (esc) (glob)

  $ flight desktop kill "$(flight desktop list | grep "kde" | cut  -f1)"
