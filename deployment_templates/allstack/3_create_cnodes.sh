...
openstack_cnode_base_file="base.yaml"
computetemplate="changestack.yaml" # may need to depend on the cluster name or else wouldn't work so well if launching nodes on the same platform in parallel


# openstack

# openstack make compute node template
cat $openstack_cnode_base_file > "$computetemplate"
...
# make cluster
openstack stack create --wait --template "$computetemplate" --parameter "key_name=$keyname" --parameter "flavor=$computesize" --parameter "image=$srcimage" --parameter "login_node_ip=$login_private_ip" --parameter "login_node_key=$login_root_contents" "$compute_stackname" --parameter "custom_data=$openstack_cloudinit" --parameter "disk_size=$computedisksize"; result=$?
...
az deployment group create --name "$DEPLOYNAME" --debug --resource-group "$azure_resourcegroup" --template-file "$computetemplate" --parameters sourceimage="$srcimage" clustername="$DEPLOYNAME" customdatanode="$azure_cnodescript_based" computeNodesCount=2
...
# aws wait for nodes to be ssh able
...
# openstack wait for nodes to be sshable

echoplus -v 2 "Attempting to connect with SSH to compute nodes."
for x in `seq 1 $cnode_count`; do
  nodelogin_public_ip=$(openstack stack output show "$compute_stackname" "node${x}_public_ip" -f shell | grep "output_value")
  nodelogin_public_ip=${nodelogin_public_ip#*\"} #removes stuff upto // from begining
  nodelogin_public_ip=${nodelogin_public_ip%\"*} #removes stuff from / all the way to end
  until ssh -i "$keyfile" -q -o 'StrictHostKeyChecking=no' "flight@$nodelogin_public_ip" 'exit'; do
...
for x in `seq 1 $cnode_count`; do
  ...
  nodelogin_private_ip=$(openstack stack output show "$compute_stackname" "node${x}_ip" -f shell | grep "output_value")
  nodelogin_private_ip=${nodelogin_private_ip#*\"} #removes stuff upto // from begining
  nodelogin_private_ip=${nodelogin_private_ip%\"*} #removes stuff from / all the way to end

  echoplus -v 0 "node${x}_public_ip=$nodelogin_public_ip"
  echoplus -v 0 "node${x}_private_ip=$nodelogin_private_ip"
  all_nodes_login_private_ip+=("$nodelogin_private_ip")
done